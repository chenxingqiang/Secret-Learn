...
"""
Federated Learning adapter for ExtraTreeRegressor

ExtraTreeRegressor is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.tree import ExtraTreeRegressor
    USING_XLEARN = True
except ImportError:
...
class FLExtraTreeRegressor:
    """Federated Learning ExtraTreeRegressor (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: ExtraTreeRegressor(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLExtraTreeRegressor with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated ExtraTreeRegressor training (supervised, non-iterative)")
