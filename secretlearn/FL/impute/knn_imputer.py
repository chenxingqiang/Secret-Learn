...
"""
Federated Learning adapter for KNNImputer

KNNImputer is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.impute import KNNImputer
    USING_XLEARN = True
except ImportError:
...
class FLKNNImputer:
    """Federated Learning KNNImputer (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: KNNImputer(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLKNNImputer with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated KNNImputer training (supervised, non-iterative)")
