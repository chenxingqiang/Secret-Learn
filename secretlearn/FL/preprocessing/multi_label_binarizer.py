...
"""
Federated Learning adapter for MultiLabelBinarizer

MultiLabelBinarizer is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.preprocessing import MultiLabelBinarizer
    USING_XLEARN = True
except ImportError:
...
class FLMultiLabelBinarizer:
    """Federated Learning MultiLabelBinarizer (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: MultiLabelBinarizer(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLMultiLabelBinarizer with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated MultiLabelBinarizer training (supervised, non-iterative)")
