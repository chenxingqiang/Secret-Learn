...
"""
Federated Learning adapter for Lars

Lars is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import Lars
    USING_XLEARN = True
except ImportError:
...
class FLLars:
    """Federated Learning Lars (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: Lars(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLLars with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated Lars training (supervised, non-iterative)")
