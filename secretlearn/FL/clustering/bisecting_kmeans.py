...
"""
Federated Learning adapter for BisectingKMeans

BisectingKMeans is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.cluster import BisectingKMeans
    USING_XLEARN = True
except ImportError:
...
class FLBisectingKMeans:
    """Federated Learning BisectingKMeans (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: BisectingKMeans(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLBisectingKMeans with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated BisectingKMeans training (supervised, non-iterative)")
