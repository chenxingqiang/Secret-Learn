...
"""
Federated Learning adapter for SpectralBiclustering

SpectralBiclustering is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.cluster import SpectralBiclustering
    USING_XLEARN = True
except ImportError:
...
class FLSpectralBiclustering:
    """Federated Learning SpectralBiclustering (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: SpectralBiclustering(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLSpectralBiclustering with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated SpectralBiclustering training (supervised, non-iterative)")
