...
"""
Federated Learning adapter for KernelDensity

KernelDensity is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.neighbors import KernelDensity
    USING_XLEARN = True
except ImportError:
...
class FLKernelDensity:
    """Federated Learning KernelDensity (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: KernelDensity(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLKernelDensity with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated KernelDensity training (supervised, non-iterative)")
