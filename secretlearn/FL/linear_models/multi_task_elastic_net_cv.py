...
"""
Federated Learning adapter for MultiTaskElasticNetCV

MultiTaskElasticNetCV is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import MultiTaskElasticNetCV
    USING_XLEARN = True
except ImportError:
...
class FLMultiTaskElasticNetCV:
    """Federated Learning MultiTaskElasticNetCV (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: MultiTaskElasticNetCV(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLMultiTaskElasticNetCV with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated MultiTaskElasticNetCV training (supervised, non-iterative)")
