...
"""
Federated Learning adapter for PoissonRegressor

PoissonRegressor is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import PoissonRegressor
    USING_XLEARN = True
except ImportError:
...
class FLPoissonRegressor:
    """Federated Learning PoissonRegressor (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: PoissonRegressor(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLPoissonRegressor with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated PoissonRegressor training (supervised, non-iterative)")
