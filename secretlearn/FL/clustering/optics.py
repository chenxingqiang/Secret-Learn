...
"""
Federated Learning adapter for OPTICS

OPTICS is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.cluster import OPTICS
    USING_XLEARN = True
except ImportError:
...
class FLOPTICS:
    """Federated Learning OPTICS (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: OPTICS(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLOPTICS with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated OPTICS training (supervised, non-iterative)")
