...
"""
Federated Learning adapter for LarsCV

LarsCV is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import LarsCV
    USING_XLEARN = True
except ImportError:
...
class FLLarsCV:
    """Federated Learning LarsCV (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: LarsCV(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLLarsCV with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated LarsCV training (supervised, non-iterative)")
