...
"""
Federated Learning adapter for MissingIndicator

MissingIndicator is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.impute import MissingIndicator
    USING_XLEARN = True
except ImportError:
...
class FLMissingIndicator:
    """Federated Learning MissingIndicator (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: MissingIndicator(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLMissingIndicator with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated MissingIndicator training (supervised, non-iterative)")
