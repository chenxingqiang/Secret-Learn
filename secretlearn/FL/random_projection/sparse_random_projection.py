...
"""
Federated Learning adapter for SparseRandomProjection

SparseRandomProjection is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.random_projection import SparseRandomProjection
    USING_XLEARN = True
except ImportError:
...
class FLSparseRandomProjection:
    """Federated Learning SparseRandomProjection (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: SparseRandomProjection(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLSparseRandomProjection with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated SparseRandomProjection training (supervised, non-iterative)")
