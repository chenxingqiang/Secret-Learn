...
"""
Federated Learning adapter for SelfTrainingClassifier

SelfTrainingClassifier is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.semi_supervised import SelfTrainingClassifier
    USING_XLEARN = True
except ImportError:
...
class FLSelfTrainingClassifier:
    """Federated Learning SelfTrainingClassifier (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: SelfTrainingClassifier(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLSelfTrainingClassifier with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated SelfTrainingClassifier training (supervised, non-iterative)")
