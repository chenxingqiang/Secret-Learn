...
"""
Federated Learning adapter for FeatureHasher

FeatureHasher is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.feature_extraction import FeatureHasher
    USING_XLEARN = True
except ImportError:
...
class FLFeatureHasher:
    """Federated Learning FeatureHasher (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: FeatureHasher(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLFeatureHasher with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated FeatureHasher training (supervised, non-iterative)")
