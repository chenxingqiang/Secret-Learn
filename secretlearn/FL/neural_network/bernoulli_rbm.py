...
"""
Federated Learning adapter for BernoulliRBM

BernoulliRBM is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.neural_network import BernoulliRBM
    USING_XLEARN = True
except ImportError:
...
class FLBernoulliRBM:
    """Federated Learning BernoulliRBM (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: BernoulliRBM(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLBernoulliRBM with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated BernoulliRBM training (supervised, non-iterative)")
