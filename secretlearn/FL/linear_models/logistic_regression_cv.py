...
"""
Federated Learning adapter for LogisticRegressionCV

LogisticRegressionCV is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import LogisticRegressionCV
    USING_XLEARN = True
except ImportError:
...
class FLLogisticRegressionCV:
    """Federated Learning LogisticRegressionCV (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: LogisticRegressionCV(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLLogisticRegressionCV with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated LogisticRegressionCV training (supervised, non-iterative)")
