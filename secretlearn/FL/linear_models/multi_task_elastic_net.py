...
"""
Federated Learning adapter for MultiTaskElasticNet

MultiTaskElasticNet is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import MultiTaskElasticNet
    USING_XLEARN = True
except ImportError:
...
class FLMultiTaskElasticNet:
    """Federated Learning MultiTaskElasticNet (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: MultiTaskElasticNet(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLMultiTaskElasticNet with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated MultiTaskElasticNet training (supervised, non-iterative)")
