...
"""
Federated Learning adapter for StackingClassifier

StackingClassifier is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.ensemble import StackingClassifier
    USING_XLEARN = True
except ImportError:
...
class FLStackingClassifier:
    """Federated Learning StackingClassifier (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: StackingClassifier(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLStackingClassifier with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated StackingClassifier training (supervised, non-iterative)")
