...
"""
Federated Learning adapter for KNeighborsTransformer

KNeighborsTransformer is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.neighbors import KNeighborsTransformer
    USING_XLEARN = True
except ImportError:
...
class FLKNeighborsTransformer:
    """Federated Learning KNeighborsTransformer (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: KNeighborsTransformer(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLKNeighborsTransformer with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated KNeighborsTransformer training (supervised, non-iterative)")
