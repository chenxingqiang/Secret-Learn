...
"""
Federated Learning adapter for SelectFdr

SelectFdr is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.feature_selection import SelectFdr
    USING_XLEARN = True
except ImportError:
...
class FLSelectFdr:
    """Federated Learning SelectFdr (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: SelectFdr(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLSelectFdr with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated SelectFdr training (supervised, non-iterative)")
