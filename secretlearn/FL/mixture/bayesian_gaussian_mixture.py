...
"""
Federated Learning adapter for BayesianGaussianMixture

BayesianGaussianMixture is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.mixture import BayesianGaussianMixture
    USING_XLEARN = True
except ImportError:
...
class FLBayesianGaussianMixture:
    """Federated Learning BayesianGaussianMixture (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: BayesianGaussianMixture(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLBayesianGaussianMixture with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated BayesianGaussianMixture training (supervised, non-iterative)")
