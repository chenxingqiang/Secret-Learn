...
"""
Federated Learning adapter for PolynomialCountSketch

PolynomialCountSketch is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.kernel_approximation import PolynomialCountSketch
    USING_XLEARN = True
except ImportError:
...
class FLPolynomialCountSketch:
    """Federated Learning PolynomialCountSketch (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: PolynomialCountSketch(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLPolynomialCountSketch with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated PolynomialCountSketch training (supervised, non-iterative)")
