...
"""
Federated Learning adapter for HDBSCAN

HDBSCAN is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.cluster import HDBSCAN
    USING_XLEARN = True
except ImportError:
...
class FLHDBSCAN:
    """Federated Learning HDBSCAN (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: HDBSCAN(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLHDBSCAN with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated HDBSCAN training (supervised, non-iterative)")
