...
"""
Federated Learning adapter for OneHotEncoder

OneHotEncoder is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.preprocessing import OneHotEncoder
    USING_XLEARN = True
except ImportError:
...
class FLOneHotEncoder:
    """Federated Learning OneHotEncoder (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: OneHotEncoder(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLOneHotEncoder with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated OneHotEncoder training (supervised, non-iterative)")
