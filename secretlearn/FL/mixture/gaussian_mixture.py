...
"""
Federated Learning adapter for GaussianMixture

GaussianMixture is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.mixture import GaussianMixture
    USING_XLEARN = True
except ImportError:
...
class FLGaussianMixture:
    """Federated Learning GaussianMixture (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: GaussianMixture(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLGaussianMixture with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated GaussianMixture training (supervised, non-iterative)")
