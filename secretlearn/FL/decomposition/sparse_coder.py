...
"""
Federated Learning adapter for SparseCoder

SparseCoder is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.decomposition import SparseCoder
    USING_XLEARN = True
except ImportError:
...
class FLSparseCoder:
    """Federated Learning SparseCoder (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: SparseCoder(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLSparseCoder with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated SparseCoder training (supervised, non-iterative)")
