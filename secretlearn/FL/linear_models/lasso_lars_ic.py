...
"""
Federated Learning adapter for LassoLarsIC

LassoLarsIC is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import LassoLarsIC
    USING_XLEARN = True
except ImportError:
...
class FLLassoLarsIC:
    """Federated Learning LassoLarsIC (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: LassoLarsIC(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLLassoLarsIC with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated LassoLarsIC training (supervised, non-iterative)")
