...
"""
Federated Learning adapter for BaseRandomProjection

BaseRandomProjection is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.random_projection import BaseRandomProjection
    USING_XLEARN = True
except ImportError:
...
class FLBaseRandomProjection:
    """Federated Learning BaseRandomProjection (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: BaseRandomProjection(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLBaseRandomProjection with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated BaseRandomProjection training (supervised, non-iterative)")
