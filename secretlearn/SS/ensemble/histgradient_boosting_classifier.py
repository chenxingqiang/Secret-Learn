...
"""
Federated Learning adapter for HistGradientBoostingClassifier
...
class SSHistGradientBoostingClassifier:
    """Federated Learning HistGradientBoostingClassifier"""
    
    def __init__(self, spu: Optional[SPU] = None, **kwargs):
        if not SECRETFLOW_AVAILABLE:
            raise RuntimeError("SecretFlow not installed")
        self.spu = spu
        self.spu = spu
        self.local_model = spu(lambda **kw: HistGradientBoostingClassifier(**kw))(**kwargs)
        self._is_fitted = False
        if USING_XLEARN:
            logging.info(f"[FL] SSHistgradientboostingclassifier with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame] = None):
        ...
        if True:
            if device in x.partitions:
                ...
                logging.info(f"[FL] Party '{party_name}' trained")
