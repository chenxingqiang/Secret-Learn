...
"""
Federated Learning adapter for RandomTreesEmbedding

RandomTreesEmbedding is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.ensemble import RandomTreesEmbedding
    USING_XLEARN = True
except ImportError:
...
class FLRandomTreesEmbedding:
    """Federated Learning RandomTreesEmbedding (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: RandomTreesEmbedding(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLRandomTreesEmbedding with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated RandomTreesEmbedding training (supervised, non-iterative)")
