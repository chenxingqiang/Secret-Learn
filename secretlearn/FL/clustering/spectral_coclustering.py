...
"""
Federated Learning adapter for SpectralCoclustering
...
try:
    from secretlearn.cluster import SpectralCoclustering
    USING_XLEARN = True
except ImportError:
...
class FLSpectralCoclustering:
    """Federated Learning SpectralCoclustering (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: SpectralCoclustering(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLSpectralCoclustering with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated SpectralCoclustering training (supervised, non-iterative)")
