...
"""
Federated Learning adapter for FunctionTransformer

FunctionTransformer is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.preprocessing import FunctionTransformer
    USING_XLEARN = True
except ImportError:
...
class FLFunctionTransformer:
    """Federated Learning FunctionTransformer (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: FunctionTransformer(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLFunctionTransformer with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated FunctionTransformer training (supervised, non-iterative)")
