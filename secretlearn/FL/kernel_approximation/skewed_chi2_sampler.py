...
"""
Federated Learning adapter for SkewedChi2Sampler

SkewedChi2Sampler is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.kernel_approximation import SkewedChi2Sampler
    USING_XLEARN = True
except ImportError:
...
class FLSkewedChi2Sampler:
    """Federated Learning SkewedChi2Sampler (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: SkewedChi2Sampler(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLSkewedChi2Sampler with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated SkewedChi2Sampler training (supervised, non-iterative)")
