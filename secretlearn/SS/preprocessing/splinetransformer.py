...
"""
Federated Learning adapter for SplineTransformer

SplineTransformer is an UNSUPERVISED preprocessing algorithm.
Data aggregated to SPU with full MPC protection.
SPU-based secure aggregation of statistics.
...
class FLSplineTransformer:
    """
    Federated Learning SplineTransformer
    
    SplineTransformer is an unsupervised preprocessing algorithm.
    
    In SS mode:
    - All data aggregated to SPU (Secure Processing Unit) for encrypted computation
    - Statistics are securely aggregated via SPU encryption
    ...
    **kwargs
        Parameters passed to SplineTransformer
    ...
    def __init__(
        ...
        **kwargs
        ...
        if USING_XLEARN:
            logging.info("[FL] FLSplineTransformer initialized with JAX acceleration")
        else:
            logging.info("[FL] FLSplineTransformer initialized with sklearn")
    ...
    def _create_local_model(**kwargs):
        """Create local SplineTransformer instance"""
        return SplineTransformer(**kwargs)
    
    def fit(self, x: Union[FedNdarray, VDataFrame]):
        """
        Fit the federated SplineTransformer
        ...
        Returns
        -------
        self : FLSplineTransformer
            Fitted preprocessor
        """
        if isinstance(x, VDataFrame):
            x = x.values
        
        logging.info("[FL] Starting federated SplineTransformer fitting (unsupervised)")
        ...
        logging.info("[FL] Federated SplineTransformer fitting completed")
        return self
