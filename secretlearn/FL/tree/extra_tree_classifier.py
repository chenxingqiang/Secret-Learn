...
"""
Federated Learning adapter for ExtraTreeClassifier

ExtraTreeClassifier is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.tree import ExtraTreeClassifier
    USING_XLEARN = True
except ImportError:
...
class FLExtraTreeClassifier:
    """Federated Learning ExtraTreeClassifier (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: ExtraTreeClassifier(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLExtraTreeClassifier with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated ExtraTreeClassifier training (supervised, non-iterative)")
