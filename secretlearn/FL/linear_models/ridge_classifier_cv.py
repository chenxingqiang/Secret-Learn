...
"""
Federated Learning adapter for RidgeClassifierCV

RidgeClassifierCV is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import RidgeClassifierCV
    USING_XLEARN = True
except ImportError:
...
class FLRidgeClassifierCV:
    """Federated Learning RidgeClassifierCV (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: RidgeClassifierCV(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLRidgeClassifierCV with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated RidgeClassifierCV training (supervised, non-iterative)")
