...
"""
Federated Learning adapter for RBFSampler

RBFSampler is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.kernel_approximation import RBFSampler
    USING_XLEARN = True
except ImportError:
...
class FLRBFSampler:
    """Federated Learning RBFSampler (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: RBFSampler(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLRBFSampler with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated RBFSampler training (supervised, non-iterative)")
