...
SPU-based secure aggregation of anomaly scores.
...
class FLIsolationForest:
    """
    Federated Learning IsolationForest
    ...
    - Scores are securely aggregated via SPU encryption
    ...
    **kwargs
    ...
    def __init__(
        ...
        **kwargs
        ...
        if USING_XLEARN:
            logging.info("[FL] FLIsolationForest initialized with JAX acceleration")
        else:
            logging.info("[FL] FLIsolationForest initialized with sklearn")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Starting federated IsolationForest training (unsupervised)")
        ...
        if True:
            if device in x.partitions:
                ...
                logging.info(f"[FL] Party '{party_name}' trained on local data")
    ...
    def _secure_aggregate_scores(self, scores_list):
        """
        Securely aggregate anomaly scores using SPU encryption
        
        This implements proper privacy-preserving score aggregation.
        """
        if self.aggregation_method == 'mean':
            # Use SPU to compute encrypted mean
            # Implementation depends on SecretFlow's SPU API
            logging.info("[FL] Secure aggregation (mean) via SPU")
            # TODO: Implement actual SPU-encrypted aggregation
            ...
            logging.info("[FL] Secure aggregation (max) via SPU")
            # Use SPU to find maximum score
            aggregator = SecureAggregator(device=self.spu)
    ...
    def _secure_aggregate_predictions(self, predictions_list):
        """Securely aggregate binary predictions using SPU"""
        logging.info("[FL] Secure prediction aggregation via SPU")
        # Majority voting with SPU encryption
        aggregator = SecureAggregator(device=self.spu)
