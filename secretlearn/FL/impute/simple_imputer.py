...
"""
Federated Learning adapter for SimpleImputer

SimpleImputer is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.impute import SimpleImputer
    USING_XLEARN = True
except ImportError:
...
class FLSimpleImputer:
    """Federated Learning SimpleImputer (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: SimpleImputer(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLSimpleImputer with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated SimpleImputer training (supervised, non-iterative)")
