...
"""
Federated Learning adapter for SelectFwe
...
try:
    from secretlearn.feature_selection import SelectFwe
    USING_XLEARN = True
except ImportError:
...
class FLSelectFwe:
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: SelectFwe(**kw))(**kwargs)
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated SelectFwe training (supervised, non-iterative)")
