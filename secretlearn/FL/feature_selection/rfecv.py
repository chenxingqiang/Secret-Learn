...
"""
Federated Learning adapter for RFECV

RFECV is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.feature_selection import RFECV
    USING_XLEARN = True
except ImportError:
...
class FLRFECV:
    """Federated Learning RFECV (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: RFECV(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLRFECV with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated RFECV training (supervised, non-iterative)")
