...
"""
Federated Learning adapter for MultiTaskLasso

MultiTaskLasso is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import MultiTaskLasso
    USING_XLEARN = True
except ImportError:
...
class FLMultiTaskLasso:
    """Federated Learning MultiTaskLasso (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: MultiTaskLasso(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLMultiTaskLasso with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated MultiTaskLasso training (supervised, non-iterative)")
