...
"""
Federated Learning adapter for NearestNeighbors

NearestNeighbors is a SUPERVISED non-iterative algorithm.
...
class FLNearestNeighbors:
    """Federated Learning NearestNeighbors (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: NearestNeighbors(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLNearestNeighbors with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated NearestNeighbors training (supervised, non-iterative)")
