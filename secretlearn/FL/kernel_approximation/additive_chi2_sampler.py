...
"""
Federated Learning adapter for AdditiveChi2Sampler

AdditiveChi2Sampler is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.kernel_approximation import AdditiveChi2Sampler
    USING_XLEARN = True
except ImportError:
...
class FLAdditiveChi2Sampler:
    """Federated Learning AdditiveChi2Sampler (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: AdditiveChi2Sampler(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLAdditiveChi2Sampler with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated AdditiveChi2Sampler training (supervised, non-iterative)")
