...
"""
Federated Learning adapter for SelectPercentile

SelectPercentile is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.feature_selection import SelectPercentile
    USING_XLEARN = True
except ImportError:
...
class FLSelectPercentile:
    """Federated Learning SelectPercentile (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: SelectPercentile(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLSelectPercentile with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated SelectPercentile training (supervised, non-iterative)")
