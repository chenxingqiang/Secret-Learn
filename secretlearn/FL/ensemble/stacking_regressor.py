...
"""
Federated Learning adapter for StackingRegressor
...
try:
    from secretlearn.ensemble import StackingRegressor
    USING_XLEARN = True
except ImportError:
...
class FLStackingRegressor:
    """Federated Learning StackingRegressor (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: StackingRegressor(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLStackingRegressor with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated StackingRegressor training (supervised, non-iterative)")
