...
"""
Federated Learning adapter for QuantileRegressor

QuantileRegressor is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import QuantileRegressor
    USING_XLEARN = True
except ImportError:
...
class FLQuantileRegressor:
    """Federated Learning QuantileRegressor (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: QuantileRegressor(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLQuantileRegressor with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated QuantileRegressor training (supervised, non-iterative)")
