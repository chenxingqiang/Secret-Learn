...
"""
Federated Learning adapter for StandardScaler

StandardScaler is an UNSUPERVISED transformation algorithm.
...
try:
    from secretlearn.discriminant_analysis import StandardScaler
    USING_XLEARN = True
except ImportError:
...
class FLStandardScaler:
    """Federated Learning StandardScaler (Unsupervised)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, aggregation_method: str = 'mean', **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: StandardScaler(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLStandardScaler with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated StandardScaler training (unsupervised)")
