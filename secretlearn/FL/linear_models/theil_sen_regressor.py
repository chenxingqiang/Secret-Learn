...
"""
Federated Learning adapter for TheilSenRegressor

TheilSenRegressor is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import TheilSenRegressor
    USING_XLEARN = True
except ImportError:
...
class FLTheilSenRegressor:
    """Federated Learning TheilSenRegressor (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: TheilSenRegressor(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLTheilSenRegressor with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated TheilSenRegressor training (supervised, non-iterative)")
