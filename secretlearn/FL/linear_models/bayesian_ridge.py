...
"""
Federated Learning adapter for BayesianRidge

BayesianRidge is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import BayesianRidge
    USING_XLEARN = True
except ImportError:
...
class FLBayesianRidge:
    """Federated Learning BayesianRidge (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: BayesianRidge(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLBayesianRidge with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated BayesianRidge training (supervised, non-iterative)")
