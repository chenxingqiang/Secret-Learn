...
"""
Federated Learning adapter for SparsePCA

SparsePCA is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.decomposition import SparsePCA
    USING_XLEARN = True
except ImportError:
...
class FLSparsePCA:
    """Federated Learning SparsePCA (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: SparsePCA(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLSparsePCA with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated SparsePCA training (supervised, non-iterative)")
