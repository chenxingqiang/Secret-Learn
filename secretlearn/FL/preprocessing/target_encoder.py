...
"""
Federated Learning adapter for TargetEncoder

TargetEncoder is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.preprocessing import TargetEncoder
    USING_XLEARN = True
except ImportError:
...
class FLTargetEncoder:
    """Federated Learning TargetEncoder (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: TargetEncoder(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLTargetEncoder with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated TargetEncoder training (supervised, non-iterative)")
