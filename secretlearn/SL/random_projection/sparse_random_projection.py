...
"""
Split Learning adapter for SparseRandomProjection
...
class SLSparseRandomProjection:
    """Split Learning SparseRandomProjection (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: PYU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SL] SLSparseRandomProjection with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SL] SLSparseRandomProjection training in PYU")
