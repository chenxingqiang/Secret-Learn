...
"""
Split Learning adapter for LabelBinarizer
...
try:
    from xlearn.naive_bayes import LabelBinarizer
    USING_XLEARN = True
except ImportError:
...
class SLLabelBinarizer:
    """Split Learning LabelBinarizer (Supervised, Non-iterative)"""
    ...
    def __init__(self, spu: PYU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SL] SLLabelBinarizer with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SL] SLLabelBinarizer training in PYU")
