...
"""
Federated Learning adapter for KernelCenterer

KernelCenterer is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.preprocessing import KernelCenterer
    USING_XLEARN = True
except ImportError:
...
class FLKernelCenterer:
    """Federated Learning KernelCenterer (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: KernelCenterer(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLKernelCenterer with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated KernelCenterer training (supervised, non-iterative)")
