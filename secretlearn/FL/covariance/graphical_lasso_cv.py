...
"""
Federated Learning adapter for GraphicalLassoCV
...
try:
    from secretlearn.covariance import GraphicalLassoCV
    USING_XLEARN = True
except ImportError:
...
class FLGraphicalLassoCV:
    """Federated Learning GraphicalLassoCV (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: GraphicalLassoCV(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLGraphicalLassoCV with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated GraphicalLassoCV training (supervised, non-iterative)")
