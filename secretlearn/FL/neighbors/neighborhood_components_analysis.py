...
"""
Federated Learning adapter for NeighborhoodComponentsAnalysis

NeighborhoodComponentsAnalysis is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.neighbors import NeighborhoodComponentsAnalysis
    USING_XLEARN = True
except ImportError:
...
class FLNeighborhoodComponentsAnalysis:
    """Federated Learning NeighborhoodComponentsAnalysis (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: NeighborhoodComponentsAnalysis(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLNeighborhoodComponentsAnalysis with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated NeighborhoodComponentsAnalysis training (supervised, non-iterative)")
