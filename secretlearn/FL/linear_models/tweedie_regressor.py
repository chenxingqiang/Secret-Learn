...
"""
Federated Learning adapter for TweedieRegressor

TweedieRegressor is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import TweedieRegressor
    USING_XLEARN = True
except ImportError:
...
class FLTweedieRegressor:
    """Federated Learning TweedieRegressor (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: TweedieRegressor(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLTweedieRegressor with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated TweedieRegressor training (supervised, non-iterative)")
