...
"""
Federated Learning adapter for GaussianRandomProjection

GaussianRandomProjection is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.random_projection import GaussianRandomProjection
    USING_XLEARN = True
except ImportError:
...
class FLGaussianRandomProjection:
    """Federated Learning GaussianRandomProjection (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: GaussianRandomProjection(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLGaussianRandomProjection with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated GaussianRandomProjection training (supervised, non-iterative)")
