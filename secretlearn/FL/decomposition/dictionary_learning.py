...
"""
Federated Learning adapter for DictionaryLearning

DictionaryLearning is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.decomposition import DictionaryLearning
    USING_XLEARN = True
except ImportError:
...
class FLDictionaryLearning:
    """Federated Learning DictionaryLearning (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: DictionaryLearning(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLDictionaryLearning with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated DictionaryLearning training (supervised, non-iterative)")
