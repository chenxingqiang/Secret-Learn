...
"""
Federated Learning adapter for LassoLarsCV

LassoLarsCV is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import LassoLarsCV
    USING_XLEARN = True
except ImportError:
...
class FLLassoLarsCV:
    """Federated Learning LassoLarsCV (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: LassoLarsCV(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLLassoLarsCV with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated LassoLarsCV training (supervised, non-iterative)")
