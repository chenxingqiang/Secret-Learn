...
"""
Federated Learning adapter for GenericUnivariateSelect

GenericUnivariateSelect is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.feature_selection import GenericUnivariateSelect
    USING_XLEARN = True
except ImportError:
...
class FLGenericUnivariateSelect:
    """Federated Learning GenericUnivariateSelect (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: GenericUnivariateSelect(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLGenericUnivariateSelect with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated GenericUnivariateSelect training (supervised, non-iterative)")
