...
"""
Federated Learning adapter for OAS

OAS is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.covariance import OAS
    USING_XLEARN = True
except ImportError:
...
class FLOAS:
    """Federated Learning OAS (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: OAS(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLOAS with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated OAS training (supervised, non-iterative)")
