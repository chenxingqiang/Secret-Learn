...
"""
Federated Learning adapter for GammaRegressor

GammaRegressor is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import GammaRegressor
    USING_XLEARN = True
except ImportError:
...
class FLGammaRegressor:
    """Federated Learning GammaRegressor (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: GammaRegressor(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLGammaRegressor with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated GammaRegressor training (supervised, non-iterative)")
