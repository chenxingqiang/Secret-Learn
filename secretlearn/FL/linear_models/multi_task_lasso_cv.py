...
"""
Federated Learning adapter for MultiTaskLassoCV

MultiTaskLassoCV is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import MultiTaskLassoCV
    USING_XLEARN = True
except ImportError:
...
class FLMultiTaskLassoCV:
    """Federated Learning MultiTaskLassoCV (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: MultiTaskLassoCV(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLMultiTaskLassoCV with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated MultiTaskLassoCV training (supervised, non-iterative)")
