...
"""
Federated Learning adapter for LassoLars

LassoLars is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import LassoLars
    USING_XLEARN = True
except ImportError:
...
class FLLassoLars:
    """Federated Learning LassoLars (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: LassoLars(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLLassoLars with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated LassoLars training (supervised, non-iterative)")
