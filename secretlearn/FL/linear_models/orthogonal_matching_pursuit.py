...
"""
Federated Learning adapter for OrthogonalMatchingPursuit

OrthogonalMatchingPursuit is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import OrthogonalMatchingPursuit
    USING_XLEARN = True
except ImportError:
...
class FLOrthogonalMatchingPursuit:
    """Federated Learning OrthogonalMatchingPursuit (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: OrthogonalMatchingPursuit(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLOrthogonalMatchingPursuit with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated OrthogonalMatchingPursuit training (supervised, non-iterative)")
