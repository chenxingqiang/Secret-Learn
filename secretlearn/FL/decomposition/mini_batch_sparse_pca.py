...
"""
Federated Learning adapter for MiniBatchSparsePCA

MiniBatchSparsePCA is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.decomposition import MiniBatchSparsePCA
    USING_XLEARN = True
except ImportError:
...
class FLMiniBatchSparsePCA:
    """Federated Learning MiniBatchSparsePCA (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: MiniBatchSparsePCA(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLMiniBatchSparsePCA with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated MiniBatchSparsePCA training (supervised, non-iterative)")
