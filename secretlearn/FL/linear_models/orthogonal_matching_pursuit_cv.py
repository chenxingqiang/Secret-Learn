...
"""
Federated Learning adapter for OrthogonalMatchingPursuitCV

OrthogonalMatchingPursuitCV is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import OrthogonalMatchingPursuitCV
    USING_XLEARN = True
except ImportError:
...
class FLOrthogonalMatchingPursuitCV:
    """Federated Learning OrthogonalMatchingPursuitCV (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: OrthogonalMatchingPursuitCV(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLOrthogonalMatchingPursuitCV with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated OrthogonalMatchingPursuitCV training (supervised, non-iterative)")
