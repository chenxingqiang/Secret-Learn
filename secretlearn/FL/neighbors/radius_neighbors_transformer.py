...
"""
Federated Learning adapter for RadiusNeighborsTransformer

RadiusNeighborsTransformer is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.neighbors import RadiusNeighborsTransformer
    USING_XLEARN = True
except ImportError:
...
class FLRadiusNeighborsTransformer:
    """Federated Learning RadiusNeighborsTransformer (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: RadiusNeighborsTransformer(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLRadiusNeighborsTransformer with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated RadiusNeighborsTransformer training (supervised, non-iterative)")
