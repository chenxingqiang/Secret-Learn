...
"""
Federated Learning adapter for ARDRegression

ARDRegression is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.linear_model import ARDRegression
    USING_XLEARN = True
except ImportError:
...
class FLARDRegression:
    """Federated Learning ARDRegression (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: ARDRegression(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLARDRegression with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated ARDRegression training (supervised, non-iterative)")
