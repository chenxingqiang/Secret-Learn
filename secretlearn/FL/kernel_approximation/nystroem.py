...
"""
Federated Learning adapter for Nystroem

Nystroem is a SUPERVISED non-iterative algorithm.
...
class FLNystroem:
    """Federated Learning Nystroem (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: Nystroem(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLNystroem with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated Nystroem training (supervised, non-iterative)")
