...
"""
Federated Learning adapter for GraphicalLasso

GraphicalLasso is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.covariance import GraphicalLasso
    USING_XLEARN = True
except ImportError:
...
class FLGraphicalLasso:
    """Federated Learning GraphicalLasso (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: GraphicalLasso(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLGraphicalLasso with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated GraphicalLasso training (supervised, non-iterative)")
