...
"""
Federated Learning adapter for SelectFpr

SelectFpr is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.feature_selection import SelectFpr
    USING_XLEARN = True
except ImportError:
...
class FLSelectFpr:
    """Federated Learning SelectFpr (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: SelectFpr(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLSelectFpr with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated SelectFpr training (supervised, non-iterative)")
