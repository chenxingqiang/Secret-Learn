...
"""
Federated Learning adapter for DictVectorizer

DictVectorizer is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.feature_extraction import DictVectorizer
    USING_XLEARN = True
except ImportError:
...
class FLDictVectorizer:
    """Federated Learning DictVectorizer (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: DictVectorizer(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLDictVectorizer with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated DictVectorizer training (supervised, non-iterative)")
