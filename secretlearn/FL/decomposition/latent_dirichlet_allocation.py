...
"""
Federated Learning adapter for LatentDirichletAllocation

LatentDirichletAllocation is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.decomposition import LatentDirichletAllocation
    USING_XLEARN = True
except ImportError:
...
class FLLatentDirichletAllocation:
    """Federated Learning LatentDirichletAllocation (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: LatentDirichletAllocation(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLLatentDirichletAllocation with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated LatentDirichletAllocation training (supervised, non-iterative)")
