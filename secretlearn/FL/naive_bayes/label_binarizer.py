...
"""
Federated Learning adapter for LabelBinarizer

LabelBinarizer is a SUPERVISED non-iterative algorithm.
...
try:
    from secretlearn.naive_bayes import LabelBinarizer
    USING_XLEARN = True
except ImportError:
...
class FLLabelBinarizer:
    """Federated Learning LabelBinarizer (Supervised, Non-iterative)"""
    ...
    def __init__(self, devices: Dict[str, PYU], heu: Optional[HEU] = None, **kwargs):
        ...
        for party_name, device in devices.items():
            self.local_models[party_name] = device(lambda **kw: LabelBinarizer(**kw))(**kwargs)
        
        if USING_XLEARN:
            logging.info("[FL] FLLabelBinarizer with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame], y: Union[FedNdarray, VDataFrame]):
        ...
        logging.info("[FL] Federated LabelBinarizer training (supervised, non-iterative)")
