...
"""
Split Learning adapter for StandardScaler
...
try:
    from xlearn.discriminant_analysis import StandardScaler
    USING_XLEARN = True
except ImportError:
...
class SLStandardScaler:
    """Split Learning StandardScaler (Unsupervised)"""
    ...
    def __init__(self, spu: PYU, **kwargs):
        ...
        if USING_XLEARN:
            logging.info(f"[SL] SLStandardScaler with JAX acceleration")
    ...
    def fit(self, x: Union[FedNdarray, VDataFrame]):
        ...
        logging.info(f"[SL] SLStandardScaler training in PYU")
